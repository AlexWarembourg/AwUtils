...
class TCN(Layer):
    ...
    def __init__(self,
        ...
        if not isinstance(nb_filters, int):
            ...
            print('Second solution is to pip install tensorflow.keras-tcn==2.1.2 to downgrade.')
            raise Exception()
...
def build_model(embedding_matrix, num_aux_targets, loss_weight):
    ...
    x1 = TCN(TCN_UNITS, return_sequences=True, dilations=[1, 2, 4, 8, 16], name='tnc1_forward')(
        x)  # , activation = 'wavenet'
    x2 = Lambda(lambda z: K.reverse(z, axes=-1))(x)
    x2 = TCN(TCN_UNITS, return_sequences=True, dilations=[1, 2, 4, 8, 16], name='tnc1_backward')(
        x2)  # ,dilations = [1, 2, 4]
    x = add([x1, x2])
